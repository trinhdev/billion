...
Alpine.data("ProductShow", ({ product, variant, reviewCount, avgRating }) => ({
    ...
    get regularPrice() {
        ...
        if (this.hasAnyOption && !this.hasSpecialPrice) {
            return this.productPriceWithOption(productPrice);
        }
    ...
    productPriceWithOption(productPrice) {
        ...
        cartItemoptions.forEach(([key, value]) => {
            ...
            if (Array.isArray(value) && value.length !== 0) {
                const selectedOptionValues = this.product.options
                    .find(({ id }) => id === Number(key))
                    .values.filter((data) => value.includes(data.id));

                const amount = selectedOptionValues.reduce(
